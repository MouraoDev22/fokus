...
let selectedTaskElement: HTMLLIElement | null = null;
...
function createTaskElement(task: Task): HTMLLIElement {
  ...
  if (!taskDescriptionParagraph) {
    throw new Error(
      "HTMLParagraphElement(taskDescriptionParagraph) não encontrado!",
    );
  }
  ...
  if (task.completed) {
    ...
    const button: HTMLButtonElement | null = li.querySelector("button");
    if (button) {
  ...
  const editButton: HTMLButtonElement | null =
    li.querySelector(".app_button-edit");
  if (editButton) {
    editButton.addEventListener("click", (event): void => {
      ...
      if (taskIndex > -1) {
        ...
        const taskDescriptionElement: HTMLParagraphElement | null =
          li.querySelector(".app__section-task-list-item-description");
...
function removeTasks(onlyCompleted: boolean): void {
  const selector: string = onlyCompleted
    ? ".app__section-task-list-item-complete"
    : ".app__section-task-list-item";
  document.querySelectorAll(selector).forEach((element): void => {
    element.remove();
    return;
  });
...
function crudAddEventListeners(): void {
  const addTaskButton: HTMLButtonElement | null =
    document.querySelector<HTMLButtonElement>(".app__button--add-task");
  const addTaskForm: HTMLFormElement | null =
    document.querySelector<HTMLFormElement>(".app__form-add-task");
  const textarea: HTMLTextAreaElement | null =
    document.querySelector<HTMLTextAreaElement>(".app__form-textarea");
  const tasksList: HTMLUListElement | null =
    document.querySelector<HTMLUListElement>(".app__section-task-list");
  const removeCompletedTasksButton: HTMLButtonElement | null =
    document.querySelector<HTMLButtonElement>("#btn-remover-concluidas");
  const removeAllTasksButton: HTMLButtonElement | null =
  ...
  if (!removeCompletedTasksButton) {
    throw new Error(
      "HTMLButtonElement(removeCompletedTasksButton) não encontrado.",
    );
  }
  ...
  addTaskButton.addEventListener<"click">("click", (): void => {
    addTaskForm.reset();
    addTaskForm.classList.toggle("hidden");
    return;
  });

  addTaskForm.addEventListener<"submit">("submit", (event): void => {
  ...
  removeCompletedTasksButton.addEventListener<"click">("click", (): void => {
    removeTasks(true);
    return;
  });

  removeAllTasksButton.addEventListener<"click">("click", (): void => {
    removeTasks(false);
    return;
  });

  document.addEventListener("FocoFinalizado", (): void => {
    if (state.selectedTask && selectedTaskElement) {
      selectedTaskElement.classList.remove(
